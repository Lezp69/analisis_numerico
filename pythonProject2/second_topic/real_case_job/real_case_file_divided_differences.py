...
#Divided Differences
x_interpolate = 50.5
divided_differences_result = interpolate_newtons_divided_difference(x_values, y_values, x_interpolate)
print(f'divided_differences interpolation at x={x_interpolate}: {divided_differences_result}')
# Plotting
x_range = np.linspace(min(x_values), max(x_values), 50)
y_interp = [interpolate_newtons_divided_difference(x_values, y_values, x) for x in x_range]
...
plt.scatter(x_interpolate, divided_differences_result, label='Interpolated Point', color='green')
plt.xlabel('Iss. Age')
plt.ylabel('Value')
